...
export function Header() {
  const { trader, facts, kei, busy, act } = useMarket()
    ...
    <header className="sticky top-0 z-20 border-b border-line bg-floor/85 backdrop-blur">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-x-4 gap-y-2 px-4 py-2.5">
        ...
        <div className="ml-auto flex items-center gap-2.5">
          ...
          >
            {trader ? shortAddress(trader.address) : '—'}
          </span>

          <span className="font-mono text-sm tabular">
            <span className="text-gold">{formatKei(kei, 4)}</span>
          ...
          <button
            ...
            onClick={() => void act('Topping up', () => trader?.topUp() ?? Promise.resolve())}
          >
            Faucet
          </button>
